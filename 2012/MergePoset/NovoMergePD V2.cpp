...
// Exibe o conteudo de uma cadeia, representada como uma lista
	...
	vector<Vertice*>* Liu1(vector<Vertice*> *lista1,vector<Vertice*> *lista2){
		...
		int NumeroDeCaminhos = (lista1->size()-1)+(lista2->size()-1);

		vector<double> valorA(NumeroEstado);
		vector<double> valorB(NumeroEstado);
		vector<char> CaminhoA(NumeroEstado);
		vector<char> CaminhoB(NumeroEstado);		
		...
		for(int i = 1 ; i < lista1->size() ; i++)
			for(int j = 1 ; j < lista2->size() ; j++){
				if(valorA[(j-1)+ (i*lista2->size())] + custoArcoMerge(i,j,lista1,lista2,'B') < valorB[(j-1)+ (i*lista2->size())]){
					...
					CaminhoB[j+ (i*lista2->size())] = ('a');
				}
				else{
					valorB[j+ (i*lista2->size())] = valorB[(j-1) + (i*lista2->size())];
					CaminhoB[j+ (i*lista2->size())] = ('b');
				}
			...
			for(int i = 0;i < CaminhoA.size(); i++)
				if(valorA[i] < valorB[i]){				
					if(CaminhoA[i] == 'a' && A != lista1->end()){
						extLinearResultante->push_back((*A));
						A++;
					}else if (CaminhoA[i] == 'b' && B != lista2->end()){
						extLinearResultante->push_back((*B));
						B++;
					}
			}
			else{
					if(CaminhoB[i] == 'a' && A != lista1->end()){
						extLinearResultante->push_back((*A));
						A++;
					}else if (CaminhoB[i] == 'b' && B != lista2->end()){
						extLinearResultante->push_back((*B));
						B++;
					}
			}
			...
			//Preenche com o resto que falta das cadeias
			if(A != lista1->end())
				while(A != lista1->end()){
					extLinearResultante->push_back((*A));
					A++;
				}
			else if(B != lista2->end())
				while(B != lista2->end()){
					extLinearResultante->push_back((*B));
					B++;
				}
...
void checarSeEstaoTodos(vector<vector<Vertice*>> *Cadeias){
	vector<bool> JaInserido(70);
	for(int i = 0;i < 70;i++)
		JaInserido[i] = false;
	...
	for(auto itCadeia = Cadeias->begin(); itCadeia != Cadeias->end(); itCadeia++){
			...
			itElementoCadeia++;//Tirando o vertice de ajuste
			for(; itElementoCadeia != (*itCadeia).end() ; itElementoCadeia++)
				JaInserido[ (*itElementoCadeia)->getIndice()] = true;		
	}

	for(int i = 0; i < 70;i++)
		if(JaInserido[i] == false)
			cout << "ERRO\n\n\n\n\n";
}
...
int main(int argc, char **args) {
	...
	//char* nomeArquivo = "rp_n70_p0.80_s3.txt";
	...
	//cout << "\nFim das cadeias\n\nInicio Do Merge\n";
	//checarSeEstaoTodos(Cadeias);

	vector<Vertice*> *aux = &Cadeias->at(0);
