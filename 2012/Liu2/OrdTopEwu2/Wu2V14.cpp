	...
	list<Vertice*>* maiorCadeia(){
		   ...
		   while(caixas->at(0).size() > 0) {
			   ...
			   for (; it != verticeGrauZero->getListaDeEntrada()->end(); it++) {
				   ...
				   // Verificar se altura deve ser atualizada
				   ...
				   // Copiar downset de verticeGrauZero para seus antecessores
				   for (int i=0; i<getNumeroDeVertices(); i++)
					   if (verticeGrauZero->estaNoDownset(i))
						   (*it)->getInicio()->adicionarRelacao(getVertice(i));
			   }
			   caixas->at(0).remove(verticeGrauZero);
		   }
	...
	list<Vertice*>* NovaSteps1And2(){
		...
		while(caixas->at(0).size() > 0) {
			...
			for (list<Arco*>::iterator it = verticeGrauZero->getListaDeEntrada()->begin();it != verticeGrauZero->getListaDeEntrada()->end(); it++) {
					...
					// Copia os transitivos
					list<Arco*>::iterator iterador = (*it)->getFinal()->getListaDeSaida()->begin();
					while(iterador != (*it)->getFinal()->getListaDeSaida()->end()){
						(*it)->getInicio()->adicionarRelacao((*iterador)->getFinal()->getIndice());
						cout << (*it)->getInicio()->getIndice() << " " << (*it)->getFinal()->getIndice() << endl;
						iterador++;
					}		
		...
		for(int i = 0; i <  getNumeroDeVertices(); i++){
			...
			for(int j = 0; j <  getNumeroDeVertices(); j++)
				cout << " " << getVertice(i)->estaNoDownset(j);
			cout << endl;
		}
		...
		//Armazenar explicitamente todas as relações transitivas
		//Reltransitivas(); Na Steps1And2 Antiga tinha este * tentar inserir isto durante a nova para pegar a maior cadeia e ja fazer
		//as relações transitivas -OBS(Já tem pronto basta apenas confirmar se pode usar).=
		return NULL;
	}
