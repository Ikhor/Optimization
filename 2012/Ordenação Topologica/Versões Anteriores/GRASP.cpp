...
int* GRASPBL(Poset p,float alfa,int qtdRodadas){	
	int i = 0;
	int *custoTotal = new int[3];
	custoTotal[0] = INT_MAX;
	custoTotal[1] = INT_MAX;
	custoTotal[2] = INT_MAX;
	while(i < qtdRodadas){
	...
	if(auditarExtLinear(ext) == false)
		cout << "ERRO "<< endl;
	if(custoSeedFixa < custoTotal[0])
		custoTotal[0] = custoSeedFixa;
	i++;
	}
	/*
	i = 0;
	while(i < qtdRodadas){
	list<Vertice*> *ext = p.GRASP(alfa,false);
	p.LocalSearch(ext);
	int	custoSeedAle = ContabilizarExtLinear(*ext);
	if(custoSeedAle < custoTotal[1])
		custoTotal[1] = custoSeedAle;
	i++;
	}
	i = 0;
	while(i < qtdRodadas){
	list<Vertice*> *ext = p.GRASP(alfa,true);
	int	custoSemBL = ContabilizarExtLinear(*ext);
	if(custoSemBL < custoTotal[2])
		custoTotal[2] = custoSemBL;
	i++;
	}	
	*/
	return custoTotal;
