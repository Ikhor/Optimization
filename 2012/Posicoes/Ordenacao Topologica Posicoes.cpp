...
public:
	...
	list<Vertice*>* GRASP(float valorAlfa,bool seed){
		...
		//Vector de iteradores
	   	list<Vertice*>::iterator *CaixaIterator = new list<Vertice*>::iterator[getNumeroDeVertices()];
		...
		while(caixas->at(0).size() > 0) {
			...
			for(it2 = caixas->at(0).begin(); it2 != caixas->at(0).end(); it2++)
			   ...
			   //Nao mexer abaixo
				...
				verticeGrauZero->posicao = ExtLinear->size();
				delete myInfos;
				delete Melhores;
		}
		delete NaCaixa;
