...
public:
	...
	list<Vertice*>* Step3(list<Vertice*> *extensao){
		...
		while (verticesNaoIncluidos.size() > 0) {
			...
			// Verificar insercao do novo vertice no final da extensao parcial
			if (extensao->back()->obterCustoParaVertice(novo->getIndice(), custoNaPosicao) && (custoNaPosicao < custoNovoVertice)) {
				custoNovoVertice = custoNaPosicao;
				posicaoDeInsercao = extensao->end();
			}
...
int ContabilizarExtLinear(list<Vertice*> ext){
	...
	for( ; iter != --ext.end(); it++){
		...
		cout << (*it)->getIndice() << " " << (*iter)->getIndice() << " " << aux << endl; 
		custo += aux;
	}
	cout << endl;
	return (int)custo;
...
void OrdenacaoTopologica(Poset p){
	...
	cout << ContabilizarExtLinear(*ext) << endl;
}

void AlgoritmoWu2(Poset p){
...
int main() {
	char* nomeArquivo = "poset_n7_d2_s123.txt";
	Poset *p = contruirPoset(nomeArquivo);
