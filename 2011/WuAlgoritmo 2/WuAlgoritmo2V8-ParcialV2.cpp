		   ...
		   while(caixas->at(0).size() > 0) {
			   ...
			   for (list<Arco*>::iterator it = verticeGrauZero->getListaDeincidencia()->begin();
				   it != verticeGrauZero->getListaDeincidencia()->end(); it++) {
					   ...
					   list<Arco*>::iterator iterador;
					   for(iterador = (*it)->getFinal()->getListaDeAdjacencia()->begin(); iterador != (*it)->getFinal()->getListaDeAdjacencia()->end(); iterador++)
...
int main() {
	...
	for(int i = 0; i < p.getNumeroDeVertices() ; i++){
		for(int j = 0; j < p.getNumeroDeVertices(); j ++)
			cout << p.getVertice(i)->estaNoDownset(j);
	cout << endl;
	}
	getchar();
}
